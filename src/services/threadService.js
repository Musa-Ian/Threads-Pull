...
exports.extractMediaFromThread = async (url) => {
  ...
  try {
    ...
    // Launch browser with improved options for avoiding detection and serverless compatibility
    const options = {
      args: [
        '--no-sandbox',
        ...
        '--font-render-hinting=none' // Reduce resource usage
      ],
      headless: 'new',
    ...
    // Skip screenshot in production to avoid file system issues in serverless environments
    if (process.env.NODE_ENV !== 'production') {
      try {
        ...
        console.log('Could not save screenshot, continuing anyway:', error.message);
      }
    }
